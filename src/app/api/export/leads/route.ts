...
export async function GET(req: NextRequest) {
  try {
    ...
    const headers = ['ID','Created At','Status','Owner Name','Email','Phone','Postcode',
      ...
      'Vet Friendly','Reactive','Has Pain','Service','Condition',
      'Preferred Date','First Visit','Clinic','Vet','How Heard','Notes','Staff Notes']
    const lines = [headers.join(',')]
    for (const r of rows) {
      lines.push([r.id,r.created_at,r.status,r.owner_name,r.owner_email,r.owner_phone,r.post_code,
        ...
        r.clinic_name,r.attending_vet,r.how_heard,r.notes,r.staff_notes].map(esc).join(','))
    }
